...
describe Zuppler::Order do
  describe '#details' do
    it 'retries and succeed' do
      ...
      response2 = Hashie::Mash.new success?: true, status: { code: '200' }
      expect(order).to receive(:execute_get).and_return response1, response2
      expect do
        order.details
      end.not_to raise_error Zuppler::RetryError
    end
